...
contract LockedNaboxPools is Ownable {
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 dayLockBlockCount;
        uint256 monthLockBlockCount;
        IERC20 lpToken;    
        IERC20 candyToken; 
        uint256 startBlock; 
    ...
    //_minuteBlockCount  每分钟出块数量
    function add(uint256 _minuteBlockCount, IERC20 _lpToken,IERC20 _candyToken,uint256 _candyPerBlock,uint256 _amount,uint256 _startBlock,bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        // 锁定高度设置一个范围，以天为单位，凡是在这一天内领取的奖励，锁定高度均在这一天0点
        //todo 设置成变量传入跨池初始化函数，因为每个网络的出块时间不相同，eth是15秒，bsc是3秒
        poolInfo.push(PoolInfo({
            dayLockBlockCount: _minuteBlockCount * 60 * 24,                     //1天锁定的数量
            monthLockBlockCount: _minuteBlockCount * 60 * 24 * 30,              //30天锁定的数量
            lpToken: _lpToken,
    ...
    function _receive(PoolInfo storage pool, UserInfo storage user, bool asc) internal {
        ...
        if (pending > 0) {
            ...
            uint256 _unlockNumber = currentNumber.add(pool.monthLockBlockCount).div(pool.dayLockBlockCount).mul(pool.dayLockBlockCount); // 解锁高度
